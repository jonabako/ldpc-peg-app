(function (global, factory) {
    ...
    function tuiSmartSearch(getSearchFunction, searchDebounceTimeMs) {
        ...
        return function (source) { return source.pipe(operators.debounceTime(searchDebounceTimeMs), operators.scan(function (previousSearched, current) { return previousSearched !== '' && current.startsWith(previousSearched)
            ? previousSearched
            : current; }, ''), operators.distinctUntilChanged(), operators.switchMap(function (value) { return getSearchFunction(value).pipe(operators.startWith(null)); }), operators.startWith(cdk.EMPTY_ARRAY)); };
    }
    ...
    function tuiZonefulMap(project, zone) {
        return rxjs.pipe(operators.map(project), operators.distinctUntilChanged(), cdk.tuiZonefull(zone));
    }
