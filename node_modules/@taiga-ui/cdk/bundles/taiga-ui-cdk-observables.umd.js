(function (global, factory) {
    ...
    function tuiControlValue(control) {
        return new rxjs.Observable(function (subscriber) {
            ...
            return control.valueChanges.pipe(operators.startWith(control.value)).subscribe(subscriber);
        });
    }
    ...
    function tuiDragAndDropFrom(element) {
        ...
        return rxjs.concat(tuiTypedFromEvent(element, 'mousedown').pipe(operators.take(1), operators.map(function (event) { return new TuiDragState(exports.TuiDragStage.Start, event); })), rxjs.merge(tuiTypedFromEvent(ownerDocument, 'mousemove').pipe(operators.map(function (event) { return new TuiDragState(exports.TuiDragStage.Continues, event); })), tuiMouseDragFinishFrom(ownerDocument).pipe(operators.take(1), operators.map(function (event) { return new TuiDragState(exports.TuiDragStage.End, event); }), operators.endWith(null))).pipe(operators.takeWhile(miscellaneous.tuiIsPresent))).pipe(operators.repeat());
    }
    ...
    function tuiZonefree(zone) {
        return function (source) { return new rxjs.Observable(function (subscriber) { return zone.runOutsideAngular(function () { return source.subscribe(subscriber); }); }); };
    }
    function tuiZoneOptimized(zone) {
        return rxjs.pipe(tuiZonefree(zone), tuiZonefull(zone));
    }
